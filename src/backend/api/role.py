...
class RoleAPI(Resource):
    ...
    def put(self,user_id):
        ...
        if 'superadmin' not in current_user.roles:
            ...
            return {"message":"User promoted successfully to support staff!"},200
            ...
            if action=='demote':
                if 'superadmin' in user.roles:
                    ...
                    return {"message":"Super Administrator demoted successfully to admin!"},200
                
                if 'admin' in user.roles:
                    oldRole=Role.query.filter_by(name="admin").first()
                    user.roles.remove(oldRole)
                    return {"message":"Administrator demoted successfully to support staff!"},200

                if 'support_staff' in user.roles:
                    oldRole=Role.query.filter_by(name="support_staff").first()
                    user.roles.remove(oldRole)
                    return {"message":"Support staff demoted successfully to user!"},200
                
                return {"message":"User already at lowest role hierarchy!"},400
            
            elif action=='promote':
                if 'superadmin' in user.roles:
                    return {"message":"User already at highest role hierarchy!"},400
                
                if 'admin' in user.roles:
                    newRole=Role.query.filter_by(name="superadmin").first()
                    user.roles.append(newRole)
                    return {"message":"Administrator promoted successfully to Super Administrator!"},200

                if 'support_staff' in user.roles:
                    newRole=Role.query.filter_by(name="admin").first()
                    user.roles.append(newRole)
                    return {"message":"Support staff promoted successfully to administrator!"},200
